...
class Profile_Friend_Authorization(Authorization):
    def read_list(self, object_list, bundle):
        return object_list.filter(Q(profile__profile_user=bundle.request.user) | Q(user_friend__id=bundle.request.user.id) | Q(user_friend=bundle.request.user.id)).distinct()

    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(profile__profile_user=bundle.request.user) | Q(user_friend__id=bundle.request.user.id) | Q(user_friend=bundle.request.user.id)).distinct():
            return True
        else:
            return False
...
class Group_Authorization(Authorization):
    def read_list(self, object_list, bundle):
        return object_list.filter(Q(creator=bundle.request.user) | Q(group_friends__id=bundle.request.user.id) | Q(group_friends__user_friend__id=bundle.request.user.id)).distinct()

    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(creator=bundle.request.user) | Q(group_friends__id=bundle.request.user.id) | Q(group_friends__user_friend__id=bundle.request.user.id)).distinct():
            return True
        else:
            return False
...
class Group_Friend_Authorization(Authorization):
    def read_list(self, object_list, bundle):
        ...
        return object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(friend__id=bundle.request.user.id) | Q(friend__user_friend__id=bundle.request.user.id)).distinct()

    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(friend__id=bundle.request.user.id) | Q(friend__user_friend__id=bundle.request.user.id)).distinct():
            return True
        else:
            return False
...
class Expense_Authorization(Authorization):
    def read_list(self, object_list, bundle):
        return object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(payer__id=bundle.request.user.id) | Q(splitters__friend__user_friend__id=bundle.request.user.id)).distinct()

    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(payer__id=bundle.request.user.id) | Q(splitters__friend__user_friend__id=bundle.request.user.id)).distinct():
            return True
        else:
            return False
