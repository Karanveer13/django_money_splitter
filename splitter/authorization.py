...
class Profile_Authorization(Authorization):
    ...
    def update_list(self, object_list, bundle):
        return object_list.filter(Q(profile_user=bundle.request.user) | Q(profile_friends__id=bundle.request.user.id) | Q(profile_friends__user__id=bundle.request.user.id)).distinct()

    def update_detail(self, object_list, bundle):
        if object_list.filter(Q(profile_user=bundle.request.user) | Q(profile_friends__id=bundle.request.user.id) | Q(profile_friends__user__id=bundle.request.user.id)).distinct():
            return True
        else:
            return False
...
class Profile_Friend_Authorization(Authorization):
    ...
    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(profile__profile_user=bundle.request.user) | Q(user__id=bundle.request.user.id) | Q(profile__profile_friends__id=bundle.request.user.id)).distinct():
            return True
        else:
            return False
...
class Group_Friend_Authorization(Authorization):
    ...
    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(friend__id=bundle.request.user.id) | Q(friend__user__id=bundle.request.user.id) |  Q(group__group_friends__user__id=bundle.request.user.id)).distinct():
            return True
        else:
            return False
...
class Expense_Authorization(Authorization):
    def read_list(self, object_list, bundle):
        return object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(payer__id=bundle.request.user.id) | Q(splitters__friend__user__id=bundle.request.user.id) | Q(splitters__id=bundle.request.user.id)).distinct()

    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(payer__id=bundle.request.user.id) | Q(splitters__friend__user__id=bundle.request.user.id) | Q(splitters__id=bundle.request.user.id)):
            return True
        else:
            return False
    ...
    def update_detail(self, object_list, bundle):
        if object_list.filter(Q(group__creator__id=bundle.request.user.id) | Q(payer__id=bundle.request.user.id) | Q(splitters__friend__user__id=bundle.request.user.id) | Q(splitters__id=bundle.request.user.id)).distinct():
            return True
        else:
            return True
...
class Expense_Splitter_Authorization(Authorization):
    def read_list(self, object_list, bundle):
        return object_list.filter(Q(expense__group__creator__id=bundle.request.user.id) | Q(expense__payer__id=bundle.request.user.id) | Q(e_splitter__group__group_friends__user__id=bundle.request.user.id)).distinct()

    def read_detail(self, object_list, bundle):
        if object_list.filter(Q(expense__group__creator__id=bundle.request.user.id) | Q(expense__payer__id=bundle.request.user.id) |Q(e_splitter__group__group_friends__user__id=bundle.request.user.id)).distinct():
            return True
        else:
            return False

    def update_list(self, object_list, bundle):
        return object_list.filter(Q(expense__group__creator__id=bundle.request.user.id) | Q(expense__payer__id=bundle.request.user.id) | Q(e_splitter__id=bundle.request.user.id) | Q(e_splitter__friend__user=bundle.request.user.id)).distinct()

    def update_detail(self, object_list, bundle):
        if object_list.filter(Q(expense__group__creator__id=bundle.request.user.id) | Q(expense__payer__id=bundle.request.user.id) | Q(e_splitter__id=bundle.request.user.id) | Q(e_splitter__friend__user=bundle.request.user.id)).distinct():
            return True
        else:
            return False
